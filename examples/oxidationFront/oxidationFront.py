...
class OxidationSimulation:
    ...
    def _map_electric_field(self):
        ...
        centers = np.array(
            [self.cell_set.getCellCenter(i) for i in range(self.num_cells)]
        )
    ...
    def solve_step(self, dt):
        ...
        matrix = sparse.coo_matrix(
            (triplet_data, (triplet_rows, triplet_cols)), shape=(self.n_dof, self.n_dof)
        ).tocsc()
        ...
        self.oxide_fraction[subset] = np.clip(
            self.oxide_fraction[subset] + d_frac, 0.0, 1.0
        )

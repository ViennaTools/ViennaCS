...
// csScreenEnergyLoss — analytic screen-oxide energy-loss (range-reduction)
// model.
//
...
// fitted once to tabulated Rp(E) data). Only the ratio to the reference is
// used, so the substrate prefactor a_substrate cancels and k == 1 exactly at
// the reference thickness (the calibrated shapes are untouched at the
// reference).
//
//   R0    = a_screen * E0^p_screen                  range in screen at incoming
//   E0 E1(t) = ((R0 - t) / a_screen)^(1/p_screen)      residual energy after t
//   of screen k(t)  = ( E1(t) / E1(ref) )^p_substrate
...
public:
  ...
  // Projected-range scale factor k for `thickness` nm of screen, relative to
  // the calibrated reference. k == 1 exactly at the reference thickness.
  NumericType rangeScale(NumericType thickness) const {
    ...
    const NumericType den =
        std::pow(residualEnergy(refThickness_), pSubstrate_);

  ...
  if (D == 3){
      ...
      for (int i = 0; i < numberOfcellsXdirection; i++){
          for (int h = 0; h < numberOfcellsYdirection; h++){
              ...
              std::array<NumericType, 3> initialCoords{initialX, initialY, initialZ};
              int initialIndex;
              ...
              do{
                  ...
                  if(initialIndex == -1){
                      ...
                      if((*material)[initialIndex] == 0.){
                          ...
                          NumericType lateral_sigma = params.get("lateral_sigma");
                          int relevant_lateral_Cells = 3 * lateral_sigma / gridDelta;
                          int relevant_Cells = 3 * sigma / gridDelta;
                          // iterate over all the cells in y [z] direction (depth), and then
                          // iterate over all the cells in x [&y] direction to get the lateral displacement
                          for (int j = 0;j < numberOfcellsZdirection + 1; j++){
                              for (int k = 0;k < relevant_lateral_Cells + 1; k++){
                                  for (int l = 0; l < relevant_lateral_Cells + 1; l++){
                                      ...
                                      NumericType shifted_xCord = xCord - (relevant_lateral_Cells * gridDelta) / 2;
                                      NumericType shifted_yCord = yCord - (relevant_lateral_Cells * gridDelta) / 2;
                                      NumericType shifted_zCord = initialZ - zCord;
                                      //std::cout << "coord: [" << shifted_xCord << ", " << shifted_yCord << "]" <<std::endl;
                                      NumericType depth = std::cos(radians) * zCord + std::sin(radians) * (initialX - shifted_xCord);
                                      NumericType lateralDisplacement = std::sqrt(std::pow((std::cos(radians) * (initialX - shifted_xCord) - std::sin(radians) * zCord), 2) + std::pow(initialY - shifted_yCord, 2));
                                      std::array<NumericType, 3> coords{shifted_xCord, shifted_yCord, shifted_zCord};

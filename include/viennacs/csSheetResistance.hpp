...
// Masetti et al., IEEE Trans. Electron Devices 30, 764 (1983) — Table I.
//
// General form:
...
template <typename T>
struct MasettiElectronMobility {
...
//   • surfacePosition = 0  (substrate at negative coordinates; depth = surface − y)
...
public:
    ...
    explicit SheetResistance(SmartPointer<CellSet> cs) : cellSet_(std::move(cs)) {}
    ...
    void setLengthUnit(NumericType lu_cm) {
        ...
        concUnitCm3_  = (lu3 > NumericType(0)) ? (NumericType(1) / lu3)
                                                : NumericType(1e21);
    }
    ...
    NumericType compute(MobilityFunctor mobility) const {
        ...
        for (std::size_t i = 0; i + 1 < depths.size(); ++i) {
            ...
            conductance += NumericType(0.5) * dz_cm *
                           (mobility(C0) * C0 + mobility(C1) * C1);
        }
        conductance *= q;

...
inline NumericType parseNumeric(const std::unordered_map<std::string, size_t> &idx,
                                const std::vector<std::string> &cells,
                                const std::string &key,
                                NumericType defaultValue = NumericType(0)) {
  const auto it = idx.find(key);
  if (it == idx.end() || it->second >= cells.size() || cells[it->second].empty())
    return defaultValue;
  return static_cast<NumericType>(std::stod(cells[it->second]));
}

inline std::string parseString(const std::unordered_map<std::string, size_t> &idx,
                               const std::vector<std::string> &cells,
                               const std::string &key,
...
template <typename NumericType>
inline LateralStraggleModel
parseLateralModel(const std::string &value,
                  LateralStraggleModel defaultValue =
                      LateralStraggleModel::Constant) {
  const auto lowered = lower(value);
  if (lowered.empty() || lowered == "constant")
    return LateralStraggleModel::Constant;
  if (lowered == "s3" || lowered == "sentauruss3" ||
      lowered == "sentaurus_s3" || lowered == "tsuprem3")
    return LateralStraggleModel::SentaurusS3;
  if (lowered == "taurus" || lowered == "tsuprem4")
    return LateralStraggleModel::Taurus;
  if (lowered == "dios")
    return LateralStraggleModel::Dios;
  return defaultValue;
}
...
template <typename NumericType>
inline NumericType crystallineChannelingScale(
    const ImplantTableEntry<NumericType> &entry, NumericType tiltDeg,
    NumericType screenThickness, NumericType damageLevel) {
  ...
  if (entry.damageDecay > NumericType(0)) {
    scale *= std::exp(-std::max(damageLevel, NumericType(0)) *
                      entry.damageDecay);
  }
...
applyCrystallineReshaping(const ImplantTableEntry<NumericType> &entry,
  ...
  adjusted.tailParams.mu =
      adjusted.headParams.mu + blend * (adjusted.tailParams.mu - adjusted.headParams.mu);
  adjusted.tailParams.sigma = adjusted.headParams.sigma +
                              blend * (adjusted.tailParams.sigma -
                                       adjusted.headParams.sigma);
  adjusted.tailParams.beta = adjusted.headParams.beta +
                             blend * (adjusted.tailParams.beta -
                                      adjusted.headParams.beta);
  adjusted.tailParams.gamma = adjusted.headParams.gamma +
                              blend * (adjusted.tailParams.gamma -
                                       adjusted.headParams.gamma);
  adjusted.tailLateralMu = adjusted.headLateralMu +
                           blend * (adjusted.tailLateralMu -
  ...
  adjusted.tailLateralLv = adjusted.headLateralLv +
                           blend * (adjusted.tailLateralLv -
                                    adjusted.headLateralLv);
  adjusted.tailLateralDeltaSigma =
      adjusted.headLateralDeltaSigma +
      blend * (adjusted.tailLateralDeltaSigma -
               adjusted.headLateralDeltaSigma);
  adjusted.tailLateralP1 =
      adjusted.headLateralP1 + blend * (adjusted.tailLateralP1 - adjusted.headLateralP1);
  adjusted.tailLateralP2 =
      adjusted.headLateralP2 + blend * (adjusted.tailLateralP2 - adjusted.headLateralP2);
  adjusted.tailLateralP3 =
      adjusted.headLateralP3 + blend * (adjusted.tailLateralP3 - adjusted.headLateralP3);
  adjusted.tailLateralP4 =
      adjusted.headLateralP4 + blend * (adjusted.tailLateralP4 - adjusted.headLateralP4);
  adjusted.tailLateralP5 =
      adjusted.headLateralP5 + blend * (adjusted.tailLateralP5 - adjusted.headLateralP5);
  return adjusted;
}
...
public:
  ...
  void loadCSV(const std::string &fileName) {
    ...
    while (std::getline(file, line)) {
      ...
      entry.headLateralMu = impl::parseNumeric<NumericType>(
          headerIndex, cells, "headLateralMu");
...
public:
  TableDrivenImplantModel(const std::string &fileName, const std::string &species,
...
template <typename NumericType, int D>
class RecipeDrivenImplantModel final : public ImplantModel<NumericType, D> {
public:
  RecipeDrivenImplantModel(const ImplantRecipe<NumericType> &recipe,
    ...
    if (recipe.useTableLookup) {
      if (resolvedTableFileName.empty()) {
        throw std::runtime_error(
            "RecipeDrivenImplantModel requires a table file for table lookup.");
      }
      table_ = ImplantTable<NumericType>(resolvedTableFileName);
